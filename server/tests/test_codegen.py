...
def test_codegen_locally():
    ...
    print("Then run: poetry run python tests/test_codegen.py --api")

def test_codegen_api():
    """
    Test the /codegen endpoint via HTTP
    """
    url = "http://localhost:8002/codegen"
    ...
    try:
        response = requests.post(url, json=request_data, timeout=300)  # 5 minutes
        
        if response.status_code == 200:
            result = response.json()
            print("✅ Code generation successful!")
            print(f"Code ID: {result['code_id']}")
            print(f"Code saved to: {result['code_file_path']}")
